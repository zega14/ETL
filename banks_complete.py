...
def extract(data_url):
    ...
    # Assuming the required table is the second one
...
def run_queries(database_name, table_name):
    ...
    cursor.execute(f"SELECT Name, MC_EUR_Billion FROM {table_name} WHERE MC_EUR_Billion > 10")
    print(cursor.fetchall())
